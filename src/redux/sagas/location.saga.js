...
function* getCityListSaga(action) {
    try {
        const result = yield axios.get("http://localhost:8080/cities");
...
function* getDistrictListSaga(action) {
    try {
        ...
        const result = yield axios.get("http://localhost:8080/districts", {
            params: {
                parentcode: cityCode,
            },
        });
...
function* getWardListSaga(action) {
    try {
        ...
        const result = yield axios.get("http://localhost:8080/wards", {
            params: {
